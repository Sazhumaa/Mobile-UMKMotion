...
const dataProduk: Product[] = [
  ...
  },
  {
    ...
    rating: 4.8,
    sold: 2100,
    image: "https://images.unsplash.com/photo-1568649929103-28ffbefaca1e?w=400",
...
const SAMPLE_UMKM_DATA: UMKMLocation[] = [
  {
    ...
    lng: 106.8456,
    verified: true,
    description: 'Warung makan keluarga yang telah berdiri sejak 1995, menyajikan masakan tradisional Indonesia dengan cita rasa autentik. Kami menggunakan bahan-bahan segar dan resep turun-temurun.',
    isOpen: true,
    photos: [
  ...
  },
  {
    ...
    lng: 106.8224,
    verified: true,
    description: 'Galeri kerajinan tangan khas Bali yang menampilkan karya terbaik pengrajin lokal. Setiap produk dibuat dengan teknik tradisional dan perhatian terhadap detail.',
    isOpen: true,
    photos: [
  ...
  },
  {
    ...
    lng: 106.8340,
    verified: true,
    description: 'Butik fashion yang menggabungkan gaya modern dengan elemen tradisional Indonesia. Menyediakan pakaian berkualitas dengan desain eksklusif dan bahan premium.',
    isOpen: false,
    photos: [
...
const categories = [
  { id: 'all', label: 'Semua', icon: 'apps' as const, color: COLORS.primary },
  { id: 'food', label: 'Kuliner', icon: 'restaurant' as const, color: COLORS.primary },
  { id: 'fashion', label: 'Fashion', icon: 'shirt' as const, color: COLORS.primary },
  { id: 'craft', label: 'Kerajinan', icon: 'brush' as const, color: COLORS.primary },
  { id: 'electronics', label: 'Elektronik', icon: 'hardware-chip' as const, color: COLORS.primary },
];
        ...
        >
          ...
          <View className="px-6 pb-4 border-b border-gray-200">
            ...
            <Text className="text-gray-600 mt-1">
              {umkmData.length} UMKM ditemukan di sekitar Anda
            </Text>
          </View>
          ...
          >
            {umkmData.map((umkm, index) => (
              <TouchableOpacity
                key={umkm.id}
                className="bg-white rounded-2xl p-4 mb-3 shadow-lg border border-orange-100"
                onPress={() => {
                  onUMKMSelect(umkm);
                  handleClose();
                }}
                  ...
                  <View className="flex-1 ml-4">
                    ...
                    <View className="flex-row justify-between items-center mb-2">
                      ...
                      <View className={`px-2 py-1 rounded-full ${
                        umkm.isOpen ? 'bg-green-100' : 'bg-red-100'
                      }`}>
                        <Text className={`text-xs font-semibold ${
                          umkm.isOpen ? 'text-green-700' : 'text-red-700'
                        }`}>
                    ...
                    {umkm.products.length > 0 && (
                      ...
                      >
                        <View className="flex-row space-x-2">
                          {umkm.products.slice(0, 3).map(product => (
                            <View key={product.id} className="flex-row items-center bg-orange-50 px-2 py-1 rounded-full">
                              <Image 
  ...
  return (
    <View className="flex-1 bg-gray-100 relative">
      ...
      <View className="flex-1 bg-blue-50 relative overflow-hidden">
        ...
        <View className="absolute inset-0 opacity-20">
          {Array.from({ length: 20 }).map((_, i) => (
            <View key={i} className="absolute w-full h-px bg-gray-400" style={{ top: `${i * 10}%` }} />
          ))}
          {Array.from({ length: 20 }).map((_, i) => (
            <View key={i} className="absolute h-full w-px bg-gray-400" style={{ left: `${i * 10}%` }} />
          ))}
        </View>
        ...
        {umkmData.map((umkm, index) => {
          ...
          return (
            ...
            >
              <View className={`relative ${isSelected ? 'scale-110' : 'scale-100'} transition-transform duration-200`}>
                <View className={`w-12 h-12 rounded-2xl items-center justify-center shadow-lg border-2 ${
                  isSelected ? 'border-orange-500 bg-white' : 'border-white bg-white'
                }`}>
                  <View className={`w-8 h-8 rounded-xl items-center justify-center ${
                    umkm.category === 'Kuliner' ? 'bg-orange-500' :
                    umkm.category === 'Fashion' ? 'bg-pink-500' :
                    umkm.category === 'Kerajinan' ? 'bg-green-500' : 'bg-blue-500'
                  }`}>
                    <Ionicons 
                      name={
                        umkm.category === 'Kuliner' ? 'restaurant' :
                        umkm.category === 'Fashion' ? 'shirt' :
                        umkm.category === 'Kerajinan' ? 'brush' : 'cube'
                      } 
                      size={16} 
                      color="white" 
                    />
      ...
      <View className="absolute top-4 right-4 space-y-2">
        <TouchableOpacity className="w-10 h-10 bg-white rounded-full shadow-lg items-center justify-center">
...
export default function RumahUMKM() {
  ...
  // Filter UMKM data
  const filteredUMKM = SAMPLE_UMKM_DATA.filter(umkm => {
  ...
  const renderProductCard = (product: Product, index: number) => (
    ...
    >
      <TouchableOpacity 
        className="bg-white rounded-2xl p-4 mb-3 shadow-lg border border-orange-100 active:scale-95"
  ...
  const renderUMKMCard = ({ item, index }: { item: UMKMLocation; index: number }) => (
    ...
    >
      <TouchableOpacity
        className="bg-white rounded-3xl mb-4 shadow-2xl border border-orange-100 active:scale-95"
      ...
      >
        <View className="p-4">
              ...
              <View className="flex-row justify-between items-center mb-3">
                <View className="flex-row items-center">
                  <View className="bg-amber-100 px-2 py-1 rounded-full flex-row items-center">
                    ...
                    <Text className="text-sm font-semibold text-amber-900 ml-1">
                      {item.rating}
                    </Text>
          ...
          {/* Product Preview dengan ScrollView */}
  ...
  const renderMapView = () => (
    <View className="flex-1">
      ...
      {/* Floating Action Button untuk UMKM Terdekat */}
      <TouchableOpacity
        className="absolute bottom-24 right-6 bg-orange-500 w-14 h-14 rounded-2xl items-center justify-center shadow-2xl"
      ...
      <View className="absolute bottom-6 right-6 space-y-3">
        <TouchableOpacity 
          className="bg-white w-12 h-12 rounded-2xl items-center justify-center shadow-lg border border-orange-100"
          onPress={() => setShowBottomSheet(true)}
        >
    ...
    return (
      ...
      >
        ...
        <View className="relative">
          <Image 
            source={{ uri: selectedUMKM.image }} 
            className="w-full h-64"
          />
          <View className="absolute inset-0 bg-gradient-to-b from-transparent to-black/30" />
          <View className="absolute bottom-4 right-4 bg-black/70 px-4 py-2 rounded-full flex-row items-center">
          ...
          <View className="flex-row justify-between items-start mb-3">
            ...
            {selectedUMKM.verified && (
              <View className="flex-row items-center bg-green-100 px-3 py-1 rounded-full">
                ...
                <Text className="text-green-700 text-sm font-semibold ml-1">
                  Terverifikasi
                </Text>
              </View>
            )}
        ...
        {/* Rating Section */}
        <View className="flex-row justify-between items-center px-6 py-4 bg-orange-50 mx-6 rounded-2xl -mt-2">
          <View className="flex-row items-center">
        ...
        <View className="flex-row justify-between px-6 py-6">
          {[
            { icon: 'navigate', label: 'Rute', color: COLORS.primary, onPress: handleStartNavigation },
            { icon: 'call', label: 'Telepon', color: '#10B981', onPress: () => openPhone(selectedUMKM.phone) },
            { icon: 'share-social', label: 'Bagikan', color: '#3B82F6', onPress: () => {} },
            { icon: 'heart', label: 'Simpan', color: '#EC4899', onPress: () => toggleFavorite(selectedUMKM.id) },
          ].map((action, index) => (
            <TouchableOpacity 
              key={action.label}
            ...
            >
              <View 
                className="w-14 h-14 rounded-2xl items-center justify-center shadow-lg mb-2"
                style={{ backgroundColor: action.color, shadowColor: action.color }}
              >
              ...
              {[
                ...
                <View key={info.title} className={`rounded-2xl p-4 border ${info.bgColor} ${info.borderColor}`}>
                  <View className="flex-row items-start">
                    <View className="w-12 h-12 rounded-xl items-center justify-center mr-4" style={{ backgroundColor: info.color }}>
                      <Ionicons name={info.icon as any} size={24} color="white" />
                    </View>
                    <View className="flex-1">
                    ...
                    {info.title === 'Telepon' && (
                      <TouchableOpacity onPress={() => openPhone(selectedUMKM.phone)}>
          ...
          {activeTab === 'products' && (
            <View>
              ...
              {selectedUMKM.products.length > 0 ? (
                selectedUMKM.products.map((product, index) => renderProductCard(product, index))
              ) : (
            ...
            <View className="space-y-6">
              ...
              <View className="bg-gray-50 rounded-2xl p-6 border border-gray-200">
                ...
                <View className="space-y-3">
                  {Object.entries(selectedUMKM.socialMedia).map(([platform, handle]) => (
                    handle && (
                      <View key={platform} className="flex-row items-center justify-between bg-white px-4 py-3 rounded-xl">
                        <View className="flex-row items-center">
                          <Ionicons 
                            ...
                            color={
                              ...
                              platform === 'tiktok' ? '#000000' : '#6B7280'
                            } 
                          />
                          <Text className="text-gray-700 font-medium ml-3 capitalize">
                            {platform}
                          </Text>
                        </View>
            ...
            <View className="flex-row flex-wrap justify-between">
              {selectedUMKM.photos.map((photo, index) => (
                <Image
                  key={index}
                  source={{ uri: photo }}
                  className="w-48 h-32 rounded-2xl mb-4"
                />
              ))}
            </View>
          )}
    ...
    return (
      ...
      >
        <Text className="text-2xl font-bold text-gray-900 mb-2">
          Menuju {selectedUMKM.name}
        </Text>
        <Text className="text-gray-600 mb-8">
          Pilih moda transportasi yang paling nyaman untuk Anda
        </Text>
        ...
        {/* Action Button */}
        <TouchableOpacity
          className="bg-orange-500 rounded-2xl py-5 items-center shadow-2xl active:scale-95"
          style={{ shadowColor: COLORS.primary }}
  ...
  return (
    <View className="flex-1" style={{ backgroundColor: COLORS.background }}>
      ...
      <View className="bg-gradient-to-r from-orange-500 to-orange-600 pt-12 pb-4 rounded-b-3xl shadow-2xl">
        ...
        {/* Category Chips - Hanya muncul setelah filter diklik */}
        {showFilterModal && (
          <Animated.ScrollView
            horizontal
            showsHorizontalScrollIndicator={false}
      ...
      <Animated.View 
        className="absolute top-0 left-0 bottom-0 right-0 bg-white"
        style={{ transform: [{ translateX: sidebarAnim }] }}
      >
        <View className="flex-1 pt-12 bg-white">
          {/* Back Button */}
          <TouchableOpacity 
            className="absolute top-14 left-6 z-10 bg-white w-12 h-12 rounded-2xl items-center justify-center shadow-lg border border-orange-100"
